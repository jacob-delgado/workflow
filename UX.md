...
| --- | --- | --- |
| "a key it does not show does nothing" | No longer stated anywhere; the sentence the previous edition cited in `docs/content/docs/usage.md` is gone. Folds into the next row. | — |
...
| "Nothing outward facing is sent without" a last look | `internal/tui/comment.go:64` | **Yes: 18 of 18.** Every outward act waits on a preview or a confirmation; the push, `R`'s re-run of CI and `u`'s rebase share one last look, `lastLook` (`internal/tui/overlay.go:157`). |
| "a refused change must never go unseen" | `internal/tui/picker.go:312` | **Yes: 14 of 14.** Every overlay that sends a request guards it while in flight (the previous edition counted 1 of 7), and every one keeps a refusal in the overlay, where it happened, until `esc` — the merge and finish previews last, through `pinnedOutcome` (`internal/tui/failure.go:413`). |
| "Each pane fails on its own" | `docs/content/docs/usage.md:80` | Yes. `internal/tui/tui.go:160` batches six loads; each pane holds and renders its own error. |
| State is "carried by the SHAPE of a glyph rather than its color" | `internal/tui/glyphs.go:16` | Yes. `unicodeGlyphs` and `asciiGlyphs` differ in shape (`glyphs.go:31`, `:43`); `NO_COLOR` keeps bold and faint (`internal/tui/tui.go:142`). One residue: the progress spine's per-system hue is color-only, mitigated by the name or its initial. |
...
**Today.** `NO_COLOR` and `ui.color: never` keep bold and faint (`internal/tui/tui.go:142`);
...
a failure in `$EDITOR` (`internal/tui/checks.go:40`, `internal/tui/run.go:354`), edits an open pull
