...
// Opened reports whether this pull request is open. FindPullRequest also returns
// a merged pull request, so a caller that means "is there an open one" — the
// standup list, the status line, the offer to open a new one — asks this rather
// than trusting found alone.
