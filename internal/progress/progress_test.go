...
func TestStagesDeriveHowFarTheWorkHasGot(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"committed, review open, CI running": {
			work: progress.Work{
				OnFeatureBranch: true, IssueNamed: true, Commits: 3, PullRequestFound: true, CI: forge.CIRunning,
			},
		...
		"CI passed and announced": {
			work: progress.Work{
				...
				PullRequestFound: true, CI: forge.CIPassed, Announced: true,
			},
			want: []progress.State{done, done, done, done, done},
		},
		"CI failed": {
			work: progress.Work{OnFeatureBranch: true, IssueNamed: true, Commits: 1, PullRequestFound: true, CI: forge.CIFailed},
		...
		"changes requested stops review reading done": {
			work: progress.Work{
				...
				PullRequestFound: true, CI: forge.CIPassed, ChangesRequested: true,
			},
			want: []progress.State{done, done, done, oops, todo},
		},
		"a post waiting for CI": {
			work: progress.Work{
				OnFeatureBranch: true, IssueNamed: true, Commits: 1,
				PullRequestFound: true, CI: forge.CIPassed, PostPending: true,
			},
