...
func TestTheDetailShowsTheIssueInFull(t *testing.T) {
	...
	// Act
	view := newWorld().live(t, 120, 40).View().Content
...
func TestCommentsShownLimitsHowManyTheDetailDraws(t *testing.T) {
	...
	// Act
	view := fewer.live(t, 120, 40).View().Content
...
func TestCommentAgesReadAsBrieflyAsTheyCan(t *testing.T) {
	...
	for name, tt := range cases {
		t.Run(name, func(t *testing.T) {
			...
			// Act
			view := aged.live(t, 120, 40).View().Content
...
func TestTheDetailFollowsTheSelectionOnceItRests(t *testing.T) {
	...
	requireScreen(t, rested.View().Content,
		"Retries.", "reported by Fred")
...
func TestADetailForAnIssueNoLongerSelectedIsNotShown(t *testing.T) {
	...
	// Assert
	requireScreen(t, settled.View().Content,
		issueKey+" "+issueSummary, "Tokens reach the log.")
	refuseScreen(t, settled.View().Content, "Retries.", "reported by Fred")
}
...
func TestMovingBackToTheIssueShownWaitsForNothing(t *testing.T) {
	...
	requireScreen(t, back.View().Content,
		issueKey+" "+issueSummary, "Tokens reach the log.")
}
...
func TestWorkInProgressIsWhereTheInterfaceOpens(t *testing.T) {
	...
	for name, tt := range cases {
		t.Run(name, func(t *testing.T) {
			...
			// Act
			view := resuming.live(t, 120, 40).View().Content
...
func TestResumingNeverOverridesAChoice(t *testing.T) {
	...
	// Act
	reloaded := typing(t, screen, "2", "r", "1").View().Content
...
func TestCommentingPreviewsBeforeItPosts(t *testing.T) {
	...
	requireScreen(t, posted.View().Content,
		"● commented on PROJ-412")
	refuseScreen(t, posted.View().Content, "┏━ Comment on")
...
func TestACommentThatCannotBePostedSaysWhy(t *testing.T) {
	...
	for name, tt := range cases {
		t.Run(name, func(t *testing.T) {
			...
			// Act
			view := typing(t, failing.live(t, 120, 40), tt.keys...).View().Content
...
func TestAFailedReEditKeepsThePreview(t *testing.T) {
	...
	requireScreen(t, failed.View().Content,
		"┏━ Comment on PROJ-412", "keep me", "✗ the editor exited with an error")
	refuseScreen(t, failed.View().Content, "lost in the editor")
}
