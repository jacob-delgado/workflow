...
func TestTheHookSeamsReadWriteAndRunLefthook(t *testing.T) {
	...
	seams := wiring.Deps(t.Context(), config.Default(), wiring.Workspace{Root: root, Remote: ""}).Hooks
	...
	if _, configured := seams.Existing(); !configured {
		t.Error("Existing does not see the configuration just written")
	}

	ranPreCommit(t, seams, root)
}

// ranPreCommit runs pre-commit through the seam and checks the hook ran.
// lefthook skips a pre-commit job when nothing is staged, as a commit would, so
// something is.
func ranPreCommit(t *testing.T, seams tui.HookDeps, root string) {
	t.Helper()
...
func TestTheHookSeamsNeverOverwriteOrGuess(t *testing.T) {
	if !proc.Available("lefthook") {
		t.Skip("lefthook is not installed, so its seams are deliberately empty")
	}
	...
	outside := wiring.Deps(t.Context(), config.Default(), wiring.Workspace{Root: t.TempDir(), Remote: ""}).Hooks
	if found, configured := outside.Existing(); len(found) != 0 || !configured {
	...
	if err == nil {
		t.Error("Write replaced a lefthook.yml that was already there")
	}
...
func TestWithoutLefthookNoHookActionIsOffered(t *testing.T) {
	...
	seams := wiring.Deps(t.Context(), config.Default(), wiring.Workspace{Root: t.TempDir(), Remote: ""}).Hooks
